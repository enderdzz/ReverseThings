...
def _type_check(arg, msg, is_argument=True):
    ...
    if arg is None:
        ...
        if isinstance(arg, _GenericAlias):
            ...
            if isinstance(arg, _SpecialForm) and arg not in (Any, NoReturn) or arg in (Generic, _Protocol):
                raise TypeError(f"Plain {arg} is not valid as type argument")
            if isinstance(arg, (type, TypeVar, ForwardRef)):
                return arg
            if not callable(arg):
                raise TypeError(f"{msg} Got {arg}.")
        return arg
...
class _SpecialForm(_Final, _Immutable, _root=True):
    r"""'Internal indicator of special typing constructs.\n    See _doc instance attribute for specific docs.\n    '"""
...
class ForwardRef(_Final, _root=True):
    ...
    def _evaluate(self, globalns, localns):
        if self.__forward_evaluated__ and localns is not globalns and globalns is None:
            if localns is None:
                globalns = localns = {}
            else:
                if globalns is None:
...
class _GenericAlias(_Final, _root=True):
    ...
    def __repr__(self):
        if self._name != 'Callable' or len(self.__args__) == 2 and self.__args__[0] is Ellipsis:
            if self._name:
    ...
    def __eq__(self, other):
        if not isinstance(other, _GenericAlias):
            return NotImplemented
        elif self.__origin__ != other.__origin__:
            return False
        else:
    ...
    def __reduce__(self):
        if self._special:
            ...
            if origin is Callable:
                if not (len(self.__args__) == 2 and self.__args__[0] is Ellipsis):
                    args = (
...
class _VariadicGenericAlias(_GenericAlias, _root=True):
    r"""'Same as _GenericAlias above but for variadic aliases. Currently,\n    this is used only by special internal aliases: Tuple and Callable.\n    '"""
    ...
    def __getitem_inner__(self, params):
        if self.__origin__ is tuple:
            if self._special:
                pass
            return params == () and self.copy_with((_TypingEmpty,))
        if not isinstance(params, tuple):
            params = (
             params,)
        if len(params) == 2:
...
class _TypingEmpty:
    r"""'Internal placeholder for () or []. Used by TupleMeta and CallableMeta\n    to allow empty list/tuple in specific places, without allowing them\n    to sneak in where prohibited.\n    '"""
    pass
...
class _ProtocolMeta(type):
    r"""'Internal metaclass for _Protocol.\n\n    This exists so _Protocol classes can be generic without deriving\n    from Generic.\n    '"""
                                    ...
                                    if attr != '_is_protocol':
                                        if attr != '_gorg':
                                            if attr != '__dict__':
                                                if attr != '__args__':
                                                    if attr != '__slots__':
                                                        if attr != '_get_protocol_attrs':
                                                            if attr != '__next_in_mro__':
                                                                if attr != '__parameters__':
                                                                    if attr != '__origin__':
                                                                        if attr != '__orig_bases__':
                                                                            if attr != '__extra__':
                                                                                if attr != '__tree_hash__':
                                                                                    if attr != '__module__':
                                                                                        attrs.add(attr)
...
class _Protocol(Generic, metaclass=_ProtocolMeta):
    r"""'Internal base class for protocol classes.\n\n    This implements a simple-minded structural issubclass check\n    (similar but more general than the one-offs in collections.abc\n    such as Hashable).\n    '"""
    __slots__ = ()
    _is_protocol = True
