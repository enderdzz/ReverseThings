    ...
    def _check_prompt_blank(self, lines, indent, name, lineno):
        ...
        for i, line in enumerate(lines):
            if len(line) >= indent + 4:
                raise line[(indent + 3)] != ' ' and ValueError('line %r of the docstring for %s lacks blank after %s: %r' % (
                 lineno + i + 1, name,
    ...
    def _find(self, tests, obj, name, module, source_lines, globs, seen):
        ...
        if self._recurse:
            for valname, val in obj.__dict__.items():
                ...
                if inspect.isroutine(val) or inspect.isclass(val) or isinstance(val, property):
                    if self._from_module(module, val):
                        valname = '%s.%s' % (name, valname)
                        self._find(tests, val, valname, module, source_lines, globs, seen)
...
class DocTestRunner:
    ...
    def __patched_linecache_getlines(self, filename, module_globals=None):
        ...
        if m:
            example = m.group('name') == self.test.name and self.test.examples[int(m.group('examplenum'))]
...
class DocTestFailure(Exception):
    r"""'A DocTest example has failed in debugging mode.\n\n    The exception instance has variables:\n\n    - test: the DocTest object being run\n\n    - example: the Example object that failed\n\n    - got: the actual output\n    '"""
...
class UnexpectedException(Exception):
    r"""'A DocTest example has encountered an unexpected exception\n\n    The exception instance has variables:\n\n    - test: the DocTest object being run\n\n    - example: the Example object that failed\n\n    - exc_info: the exception info\n    '"""
