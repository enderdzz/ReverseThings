...
def _85encode(b, chars, chars2, pad=False, foldnuls=False, foldspaces=False):
    ...
    chunks = ['z' if foldnuls and not word else (word == 538976288 and 'y' if foldspaces else chars2[(word // 614125)] + chars2[(word // 85 % 7225)] + chars[(word % 85)]) for word in words]
    if padding:
        if not pad:
            if chunks[(-1)] == 'z':
                chunks[-1] = chars[0] * 5
...
def a85decode(b, *, foldspaces=False, adobe=False, ignorechars=' \t\n\r\x0b'):
    ...
    for x in b + 'uuuu':
        ...
        if len(curr) == 5:
            ...
            decoded_append('\x00\x00\x00\x00')
        else:
            if foldspaces:
                pass
            if x == 121:
