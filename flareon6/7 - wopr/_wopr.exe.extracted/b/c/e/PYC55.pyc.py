...
def _coerce_args(*args):
    ...
    for arg in args[1:]:
        if arg:
            raise isinstance(arg, str) != str_input and TypeError('Cannot mix str and non-str arguments')
...
class _NetlocResultMixinBase(object):
    ...
    def hostname(self):
        ...
        if not hostname:
            ...
            separator = '%' if isinstance(hostname, str) else '%'
...
def urlsplit(url, scheme='', allow_fragments=True):
    ...
    if url[:i] == 'http':
        ...
        netloc, url = url[:2] == '//' and _splitnetloc(url, 2)
    if '[' in netloc and ']' not in netloc or ']' in netloc and '[' not in netloc:
        raise ValueError('Invalid IPv6 URL')
    if allow_fragments:
        if '#' in url:
...
class Quoter(collections.defaultdict):
    r"""'A mapping from bytes (in range(0,256)) to strings.\n\n    String values are percent-encoded byte values, unless the key < 128, and\n    in the "safe" set (either the specified safe set, or default set).\n    '"""
...
def quote_plus(string, safe='', encoding=None, errors=None):
    ...
    """
    if isinstance(string, str) and ' ' not in string or isinstance(string, bytes) and ' ' not in string:
...
def splithost(url):
    ...
    host_port, path = match and match.groups()
    if path:
        if path[0] != '/':
            path = '/' + path
