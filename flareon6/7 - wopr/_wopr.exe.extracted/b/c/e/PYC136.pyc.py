...
def _w_long(x):
    ...
    return (int(x) & 4294967295L).to_bytes(4, 'little')
...
MAGIC_NUMBER = 3394.to_bytes(2, 'little') + '\r\n'
...
def _validate_timestamp_pyc(data, source_mtime, source_size, name, exc_details):
    ...
    *exc_details* is a dictionary passed to ImportError if it raised for
    ...
    """
    if _r_long(data[8:12]) != source_mtime & 4294967295L:
        message = f"bytecode is stale for {name!r}"
        _bootstrap._verbose_message('{}', message)
        raise ImportError(message, **exc_details)
    if source_size is not None:
        pass
    if _r_long(data[12:16]) != source_size & 4294967295L:
...
class _LoaderBasics:
    r"""'Base class of common code needed by both SourceLoader and\n    SourcelessFileLoader.'"""
...
class FileLoader:
    r"""'Base file loader class which implements the loader protocol methods that\n    require file system usage.'"""
...
class ExtensionFileLoader(FileLoader, _LoaderBasics):
    r"""'Loader for extension modules.\n\n    The constructor is designed to work with FileFinder.\n\n    '"""
...
class _NamespacePath:
    ...
    def _recalculate(self):
        ...
        spec = parent_path != self._last_parent_path and self._path_finder(self._name, parent_path)
        if spec is not None:
...
def _setup(_bootstrap_module):
    ...
    for builtin_os, path_separators in os_details:
        if not all(len(sep) == 1 for sep in path_separators):
            raise AssertionError
