...
def visiblename(name, all=None, obj=None):
    ...
    if name in frozenset(['__credits__', '__module__', '__spec__', '__builtins__', '__qualname__', '__file__', '__slots__', '__loader__', '__date__', '__path__', '__author__', '__package__', '__cached__', '__name__', '__version__', '__doc__']):
        return 0
    else:
...
def source_synopsis(file):
    line = file.readline()
    while not (line[:1] == '#' or line.strip()):
        line = file.readline()
        if not line:
            break
...
class Doc:
    ...
    def document(self, object, name=None, *args):
        ...
        elif inspect.ismemberdescriptor(object):
            return (self.docdata)(*args)
        try:
    ...
    def getdocloc(self, object, basedir=os.path.join(sys.base_exec_prefix, 'lib', 'python%d.%d' % sys.version_info[:2])):
        ...
        if not (object.__name__ in ('errno', 'exceptions', 'gc', 'imp', 'marshal',
                                    'posix', 'signal', 'sys', '_thread', 'zipimport') or file.startswith(basedir) and file.startswith(os.path.join(basedir, 'site-packages'))):
...
class HTMLRepr(Repr):
    ...
    def repr1(self, x, level):
        if hasattr(type(x), '__name__'):
            ...
            return hasattr(self, methodname) and getattr(self, methodname)(x, level)
        else:
...
class HTMLDoc(Doc):
        ...
        if hasattr(object, '__version__'):
            ...
            for key, value in classes:
                for base in value.__bases__:
                    ...
                    if modname != name:
                        cdict[key] = cdict[base] = module and hasattr(module, key) and getattr(module, key) is base and key not in cdict and modname + '.html#' + key
            ...
            for key, value in inspect.getmembers(object, inspect.isroutine):
                if all is not None or inspect.isbuiltin(value) or inspect.getmodule(value) is object:
                    if visiblename(key, all, object):
                        ...
                        fdict[key] = '#-' + key
                        fdict[value] = inspect.isfunction(value) and fdict[key]
    ...
    def docclass(self, object, name=None, mod=None, funcs={}, classes={}, *ignored):
        ...
        argspec = signature and str(signature)
        if argspec:
            if argspec != '()':
    ...
    def docroutine(self, object, name=None, mod=None, funcs={}, classes={}, methods={}, cl=None):
        ...
        note = ''
        skipdocs = 0
        imclass = _is_bound_method(object) and object.__self__.__class__
        if cl:
...
def describe(thing):
    ...
    elif inspect.isbuiltin(thing):
        return 'built-in function ' + thing.__name__
    elif inspect.isgetsetdescriptor(thing):
        return 'getset descriptor %s.%s.%s' % (
         thing.__objclass__.__module__, thing.__objclass__.__name__,
         thing.__name__)
    elif inspect.ismemberdescriptor(thing):
