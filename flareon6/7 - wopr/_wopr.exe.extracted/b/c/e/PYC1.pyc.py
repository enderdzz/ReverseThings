    ...
    with open(executable, 'rb') as (f):
        ...
        while pos < len(binary):
            if 'libc' in binary or 'GLIBC' in binary:
                ...
                m = None
            chunk = m and m.end() == len(binary) and f.read(chunksize)
            if chunk:
            ...
            if not lib:
                ...
                if glibc:
                    ...
                    if lib != 'glibc':
                        ...
                        if soversion:
                            pass
                if version and V(soversion) > V(version):
                    version = soversion
            if threads:
...
def uname():
    ...
    if not (no_os_uname or list(filter(None, (system, node, release, version, machine)))):
        if no_os_uname:
            ...
            machine = ''
        use_syscmd_ver = 1
        release, version, csd, ptype = system == 'win32' and win32_ver()
    if release:
        if version:
            use_syscmd_ver = 0
        if not machine:
        ...
        system, release, version = use_syscmd_ver and _syscmd_ver(system)
