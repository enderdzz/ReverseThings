...
def main():
    ...
    optional_header_size, _ = struct.unpack_from('=HH', data, 0x14) # 0xF0
    magic, _, _, size_of_code = struct.unpack_from('=HBBI', data, 0x18) # size_of_code 0xDA00
    struct.pack_into('=H', data, 0x18, 0x20b)
    print('Magic:', hex(magic))
    print('Size of code:', hex(size_of_code))

    base_of_code = struct.unpack_from('=I', data, 0x2c)[0]
    ...
    print('Base of data:', hex(base_of_data)) # 0xF000
    ...
    data_size = 0x3000
    pos = data.find(struct.pack('=IIIII', 3, 5, 7, 11, 13)) # 0x11040 - 
    print("pos:", hex(pos))
    if pos >= 0:
...
#################################################################################
    ...
    # base_of_code = 0x1000
    # base_of_data = 0xEA00
    # size_of_header = 0x0 #### NOTE!
    struct.pack_into('=8sIIIIIIHHI', data, 0x18 + optional_header_size + 0x28 * section, # 0x108
        b'.text',
